...
if($_REQUEST["submit"]){
    ...
    if($filter!=""){
        if($filter=="commingdated"){
                ...
                ->where_equal_to(array("dr_id"=>common::get_session(DR_LOGIN_USER_ID),"visited"=>"0"))
            ...
            $date = date(MYSQL_DATE_FORMAT);
                $q->select()
                ->from(TBL_Appointment)
                ->where_equal_to(array("dr_id"=>common::get_session(DR_LOGIN_USER_ID),"visited"=>"0","confirm"=>1))
                ->order_by("app_date DESC")
                ->run();
        }
    }
