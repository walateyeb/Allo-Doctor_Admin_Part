...
class Query {
    ...
    public function get_selected() {
        // retours un tableau de SELECT result(s)
        if (isset($this->limit) && 1==$this->limit) {
            // utilisé quand selection avec limite(1)
            $result = array();
            ...
            // utilisé quand selection avec non limite ou une limite > 1
            $results = array();
    ...
    public function select($select = '*') {
        // SELECT Récupère des fichiers à partir d'une ou plusieurs tables.
        $this->select = $select;
        return $this;
    }
    
    public function select_from($select, $table) {
        // alias pour select() au lieu d utiliser à la fois select() && from()
        self::select($select);
        self::from($table);
        return $this;
    }
    ...
    public function from($from) {
        // De cibler les tables spécifiées.
        $this->from = $from;
        return $this;
    }
    
    public function get_affected() {
        // Renvoie le nombre de lignes affectées par la dernière INSERT, UPDATE, REPLACE ou DELETE
    ...
    public function having($having = '', $comparison = '=', $boolean_operator = 'AND') {
        // Ayant utilisé avec GROUP BY pour spécifier les critères pour les enregistrements groupés.
        if (empty($having)) {
            $this->having = '';
        }
        else {
    ...
    public function limit($limit) {
        // LIMITE Limitez le nombre d'enregistrements sélectionnés ou supprimés.
    ...
    public function range($limit, $offset) {
        // alias au lieu d'utiliser à la fois limite () & offset ()
        self::limit($limit);
        self::offset($offset);
        return $this;
    }
    ...
    public function get($use_limit = false) {
        // retourne select, insert or update query
    ...
    private function _get_group_by() {
        // GROUP BY Détermine comment les enregistrements doivent être regroupés.
    ...
    private function _get_order_by() {
        // ORDER BY d'ordonner les enregistrements
        if (
            !isset($this->order_by) ||
            empty($this->order_by)
            ) {
    ...
    private function _get_where_not_in() {
        // NOT IN Garantit la valeur est pas dans la liste
        if (
    ...
    private function _get_where_not_like() {
        // NOT LIKE Utilisé pour comparer des chaînes
        if (
    ...
    /* Execter */
    public function run() {
        // exécute la requête, retourne résultat mysql
        if (self::get()) {
            ...
            switch ($this->query_type) {
                ...
                case 'select':
                    if (!(isset($this->page) || isset($this->offset))) {
                        ...
                        if (self::$function()) {
                            // pour pagination:
                            $this->perpage = $this->limit; // pour get_perpage()
                            $this->total = $this->results; // pour get_total()
                            // calculer pages
                            ...
                            if (!isset($this->offset)) {
                                ...
                                // calculer la page en utilisant l'offset et par page
                                // déterminer sur quelle page le décalage serait sur
                            ...
                            // mettre à jour la requête select avec limite maintenant que les pages est défini
                            self::_get_select_query(true);
                            // exécuter select requête avec limite de mise à jour et l'offset
                            return self::_run_select();
    ...
    public  function ge_fetch_array($sql)
		...
		{
			// cette fonction publique nécessite la présence de good_query() 
		...
		{
			// cette fonction publique nécessite la présence de good_query_list() 
