			...
			function upload_image_and_thumbnail($datakey, $imgscale, $thumbscale,$path, $folderName, $square,$icon='50') {
		      if(is_array($_FILES[$datakey]["name"]))
              {
                    ...
                    for($i=0;$i<count($_FILES[$datakey]["name"]);$i++)
        			{
    					...
    					$error = 0;
    					$tempuploaddir = $path."/temp"; // le  répertoire / temp /, doit supprimer l'image après que nous téléchargeons
    					$biguploaddir = $path."/".$folderName."/big"; // le répertoire /big/ 
    					$smalluploaddir = $path."/".$folderName."/small"; // le répertoire /small/  pour la vignette
    					...
    					{
    						// Obtenir la taille d'image
    						$imgsize = GetImageSize($_FILES[$datakey]['tmp_name'][$i]);
    					}
    
    					// Générer un nom unique pour l'image (de l'horodatage)
                        ...
                        {                    
    						// Copiez l'image dans le répertoire temporaire
                            if (!copy($_FILES[$datakey]['tmp_name'][$i],"$tempfile"))
                            {
    							...
    							if($square) {
    								//Générer la petite version carrée de l'image avec échelle de $thumbscale
    								$this->crop_img($tempfile, $thumbscale, $croppedfile);
                                    $this->crop_img($tempfile, $icon, $iconfile);
    							}
    							else {
    								//Générer la grande version de l'image avec un maximum de $imgscale soit dans les directions
                         ...
                         // Image téléchargée, retourner le nom du fichier
              ...
              }else
              {
                  if (strlen($_FILES[$datakey]['name'])>4){ 
    					...
    					$error = 0;
    					$tempuploaddir = $path."/temp"; 
    					$biguploaddir = $path."/".$folderName."/big"; 
    					$smalluploaddir = $path."/".$folderName."/small"; 
    					$iconuploaddir= $path."/".$folderName."/icon";
