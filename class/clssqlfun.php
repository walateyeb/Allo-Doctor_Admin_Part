...
class sqlfun
{
		...
		{
			$res=mysql_query($sql) or die("requête est erronee");
			$table = array();
		...
		{
			$res=mysql_query($sql) or die("requête est erronée");
			$table = array();
			if(mysql_num_rows($res)>0)
			{
		...
		public static function ge_table($table)
		{
			...
			if(mysql_num_rows($res)>0)
			{
				echo "<tr>";
					...
					echo "<th><a href='?key=$Primkey&action=edit' >Modifier</a></th><th><a href='?key=$Primkey&action=delete'>Supprimer</a></th>";
				echo "</tr>";
