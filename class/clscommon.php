...
<?php
                ...
                public static function seoUrl($string) {
                    //Indésirable:  {UPPERCASE} ; / ? : @ & = + $ , . ! ~ * ' ( )
                    $string = strtolower($string);
                    //éffacer  les caractères indésirables
              ...
              public static function send_mail($to,$subject,$body)
              {
                            ...
                            $mail->IsSMTP(); // la classe à utiliser SMTP
                            $mail->Host       = SMTP_SERVER; // serveur SMTP
                            $mail->SMTPDebug  = 2;                     // activer les informations de débogage SMTP (pour les tests)
                                                                       // 1 = erroeurs et msg
                                                                       // 2 = msg seulement
                            $mail->SMTPAuth   = true;                  // activer l'authentification SMTP
                            $mail->Host       = SMTP_SERVER;      // définir GMAIL comme le serveur SMTP
                            $mail->Port       = 25;                   // définir le port SMTP pour le serveur GMAIL
